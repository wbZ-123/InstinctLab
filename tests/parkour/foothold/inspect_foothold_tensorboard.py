...
def _status_for(tag: str, summary: SeriesSummary) -> str:
    ...
    if name == "foothold_planner_safe_target_valid_fraction":
        if summary.last >= 0.8:
            return "OK"
        if summary.last >= 0.5:
            return "WATCH"
    ...
    for tag in tags:
        ...
        rows.append(
            ReportRow(
                ...
                tag=tag,
                summary=summary,
                status=_status_for(tag, summary),
            )
        )
    return rows
